...
class Lab3Driver(Node):
    def __init__(self, threshold=0.2):
        ...
        self.print_twist_messages = True
    ...
    def close_enough(self):
        ...
        dist = sqrt(self.target.point.x**2 + self.target.point.y**2)
        return dist <= self.threshold

    def distance_to_target(self):
        """ Communicate with send points - set to distance to target"""
        # return large number if we aren't there
        if not self.target:
            return 999.0
    ...
    def set_target(self):
        ...
        if self.goal:
            try:
                ...
                transform = self.tf_buffer.lookup_transform('odom', 'base_link', rclpy.time.Time(), timeout=rclpy.duration.Duration(seconds=3.0))
                ...
                # Return whatever the last known target was, or wait for the next loop
                return self.target
    ...
    def get_obstacle(self, scan):
        ...
        # x, y
        y_distances = ranges * np.sin(scan_thetas)
        x_distances = ranges * np.cos(scan_thetas)

        # get in front of robot (where it has to stop)
        in_front_vals = (np.abs(y_distances) < 0.25) & (x_distances > 0.0) & (x_distances < 0.5)

        # there's an object
        if np.any(in_front_vals):
            # get objects in front of robot and find the center
            y_front = y_distances[in_front_vals]
            avg_y = np.mean(y_front)

            # turn in the direction that the object is most not in
            turn_direction = -1.0 if avg_y > 0 else 1.0
            return True, 0.05, turn_direction * (np.pi * 0.1) # have it move a little bit as it turns
        return False, 0.0, 0.0
    ...
    def get_twist(self, scan):
        ...
        # check for obstacle
        is_obstacle, obs_speed, obs_turn = self.get_obstacle(scan)
        if is_obstacle:
            t.twist.linear.x = obs_speed
            t.twist.angular.z = obs_turn
            return t

        # find angle to target and distance
        angle_to_target = atan2(self.target.point.y, self.target.point.x)
        dist_to_target = sqrt(self.target.point.x**2 + self.target.point.y**2)

        # turn speed
        t.twist.angular.z = float(np.clip(angle_to_target, -max_turn, max_turn))
        
        # forward speed
        if fabs(angle_to_target) < 0.5:
            # as before scale speed by distance to object
            desired_speed = max_speed * tanh(dist_to_target)
            t.twist.linear.x = float(np.clip(desired_speed, min_speed, max_speed))
        else:
            # don't move if we're facing away from target
            t.twist.linear.x = 0.0
